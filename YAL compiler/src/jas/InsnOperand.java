...
{
	abstract void write(ClassEnv e, CodeAttr ce, DataOutputStream out) throws IOException, jasError;
...
class UnsignedByteOperand extends InsnOperand
{
	...
	{
		if (val >= 256)
			throw new jasError("Operand is too large (" + val + ") for this instruction");
...
class TableswitchOperand extends InsnOperand
{
	...
	TableswitchOperand(Insn s, int min, int max, LabelOrOffset def, LabelOrOffset j[])
	{
		this.min = min;
		this.max = max;
		dflt = def;
...
/*
 * --- Revision History --------------------------------------------------- ---
 * Iouri Kharon, Aug 10 2006 Added 'Wide' prefix support to IincOperand and
 * UnsideWideOperand
 */
