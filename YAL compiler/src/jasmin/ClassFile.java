...
/**
 * A ClassFile object is used to represent the binary data that makes up a Java
 * class file - it also serves as the public API to the Jasmin assembler, though
 * users should beware: the API is likely to change in future versions (though
 * its such a small API at the moment that changes are likely to have only a
 * small impact).
 * <p>
 *
 * To assemble a file, you first construct a jasmin.ClassFile object, then call
 * readJasmin() to read in the contents of a Jasmin assembly file, then call
 * write() to write out the binary representation of the class file.
 * <p>
 *
...
public class ClassFile
{
	...
	void addGenericAttr(String name, String file) throws IOException, jasError
	{
		...
		if (cur_method != null)
		{
			flushInsnBuffer();
			if (code != null)
				code.addGenericAttr(gattr);
			else
				cur_method.addGenericAttr(gattr);
		} else if (cur_field != null)
		{
	...
	void addField(short access, String name, String desc, String sig, Object value) throws jasError
	{
		...
		if (sig != null)
			cur_field.setSignature(new SignatureAttr(sig));
		endField();
	}
	...
	void newMethod(String name, String descriptor, int access)
	{
		...
		stackmap = null;
		verifyframe = null;
		cur_method = new Method((short) access, new AsciiCP(name), new AsciiCP(descriptor));
	}
	...
	void plantRelativeGoto(String name, int val) throws jasError
	{
		...
		{ // forward the request for "normal" instructions
			plant(name, val);
		}
	}
	...
	void plant(String name, String val, int nargs) throws jasError
	{
		...
		if (insn.args.equals("interface"))
		{
			...
			bufferInsn(new InvokeinterfaceInsn(new InterfaceCP(split[0], split[1], split[2]), nargs));
	...
	void endLookupswitch(int defoffset) throws jasError
	{
		...
		int[] offsets = (int[]) offlabs[0];
		LabelOrOffset[] labels = (LabelOrOffset[]) offlabs[1];
		bufferInsn(new LookupswitchInsn(new LabelOrOffset(defoffset), offsets, labels));
	}
	...
	void endTableswitch(String deflabel) throws jasError
	{
		...
		bufferInsn(new TableswitchInsn(low_value, low_value + labels.length - 1, new LabelOrOffset(getLabel(deflabel)),
				labels));
	}
	...
	void endTableswitch(int defoffset) throws jasError
	{
		flushInsnBuffer();
		LabelOrOffset labels[] = checkTableswitch();
		bufferInsn(new TableswitchInsn(low_value, low_value + labels.length - 1, new LabelOrOffset(defoffset), labels));
	}
	...
	/**
	 * Makes a new ClassFile object, used to represent a Java class file. You can
	 * then use readJasmin to read in a class file stored in Jasmin assembly format.
	 */
	 ...
	 * If something goes wrong, this throws one of an IOException, or a jasError, or
	 * one of a few other exceptions. I'll tie this down more formally in the next
	 * version.
	 *
	 * @param input
	 *            is the stream containing the Jasmin assembly code for the class.
	 *
	 * @param name
	 *            is the name of the stream. This name will be concatenated to error
	 *            messages printed to System.err.
	 *
	 * @param numberLines
	 *            true if you want Jasmin to generate line numbers automatically,
	 *            based on the assembly source, or false if you are using the
	 *            ".line" directive and don't want Jasmin to help out.
	 */
	...
	/**
	 * Returns the number of warnings/errors encountered while parsing a file. 0 if
	 * everything went OK.
	 */
	...
	/**
	 * Writes the binary data for the class represented by this ClassFile object to
	 * the specified output stream, using the Java Class File format. Throws either
	 * an IOException or a jasError if something goes wrong.
	 */
...
/*
 * --- Revision History --------------------------------------------------- ---
 * Iouri Kharon, Aug 10 2006 Added 'Wide' prefix support to some instructions
 * 
 * --- Iouri Kharon, Feb 17 2006 Added extended syntax for .stack
 * 
 * --- Iouri Kharon, Dec 30 2005 Added multiline .field, directive .inner and
 * .anotation
 * 
 * --- Iouri Kharon, Dec 20 2005 Added LocalVariableTypeTable support in addVar
 * methods (called by .var)
 * 
 * --- Daniel Reynaud, Oct 22 2005 Added setSourceDebugExtension() method
 * (called by .debug) Added setEnclosingMethod() method (called by .enclosing)
 * 
 * --- Daniel Reynaud, Oct 19 2005 Added setVersion() method (called by
 * .bytecode) Changed BGN_METHOD, END_METHOD and line number label to avoid
 * collision
 * 
 * --- Jonathan Meyer, April 11 1997 Fixed bug where source_name was not being
 * set in class_env.
 * 
 * --- Jonathan Meyer, Mar 1 1997 Renamed "Jasmin" class "ClassFile".
 * 
 * --- Jonathan Meyer, Feb 8 1997 Converted to non-static. Made a public API.
 * Split off InsnInfo to a separate file.
 * 
 * --- Jonathan Meyer, Oct 1 1996 Added addInterface method, used by the
 * .implements directive.
 * 
 * --- Jonathan Meyer, July 25 1996 Added setLine and line_num, to fix problem
 * with autoNumber. Added report_error method.
 * 
