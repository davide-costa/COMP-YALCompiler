...
class Scanner
{
	...
	/*
	 * // Character can be present in signature static final String SIGCHARS =
	 * ";:()[/.^*+-<>@";
	 */
	...
	private char nameEscape() throws java.io.IOException, jasError
	{
		advance();
		if (next_char != 'u')
		 ...
		 * Character.isIdentifierIgnorable(chval))) { throw new
		 * jasError("Invalid unicode char from name/signature"); }
		 */
		return chval;
	}
...
/*
 * --- Revision History --------------------------------------------------- ---
 * Iouri Kharon, Mar 13 2006 Added support for '\\u' escape sequnce in
 * name/signature Added '' enclosed names (overload of reserved words) --- Iouri
 * Kharon, Feb 17 2006 Remove infinite loop when last line in source file do not
 * have EOL --- Iouri Kharon, Dec 19 2005 Added '\\u' escape sequence Change
 * '\octal' escape sequence Added very long string support --- Daniel Reynaud,
 * Oct 19 2005 Added '\\' escape sequence --- Jonathan Meyer, Feb 8 1997
 * Converted to be non-static --- Jonathan Meyer, Oct 30 1996 Added support for
 * more \ escapes in quoted strings (including octals). --- Jonathan Meyer, Oct
 * 1 1996 Added .interface and .implements --- Jonathan Meyer, July 25 1996
 * changed IN to IS. Added token_line_num, which is the line number of the last
 * token returned by next_token(). --- Jonathan Meyer, July 24 1996 added mods
 * to recognize '\r' as whitespace.
 */
