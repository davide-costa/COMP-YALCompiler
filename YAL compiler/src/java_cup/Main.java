...
public class Main
{
	...
	protected static void usage(String message)
	{
		...
		System.err.println();
		System.err.println("Usage: " + version.program_name + " [options]\n"
	...
	protected static void open_files()
	{
		...
		try
		{
			parser_class_file = new PrintStream(new BufferedOutputStream(new FileOutputStream(fil), 4096));
		} catch (Exception e)
		{
		...
		try
		{
			symbol_class_file = new PrintStream(new BufferedOutputStream(new FileOutputStream(fil), 4096));
		} catch (Exception e)
		{
			System.err.println("Can't open \"" + out_name + "\" for output");
			System.exit(4);
	...
	protected static void check_unused()
	{
		...
		for (Enumeration n = non_terminal.all(); n.hasMoreElements();)
		{
			...
			if (nt.use_count() == 0)
			{
				...
				if (!emit.nowarn)
				{
					System.err.println("Warning: Non terminal \"" + nt.name() + "\" was declared but never used");
					lexer.warning_count++;
				}
			}
		}
	...
	protected static void build_parser() throws internal_error
	{
		...
		{
			((lalr_state) st.nextElement()).build_table_entries(action_table, reduce_table);
		}
	...
	protected static void emit_summary(boolean output_produced)
	{
		...
		System.err.println("  " + lexer.error_count + " error" + plural(lexer.error_count) + " and "
				+ lexer.warning_count + " warning" + plural(lexer.warning_count));
		...
		/* unused symbols */
		System.err
				.println("  " + emit.unused_term + " terminal" + plural(emit.unused_term) + " declared but not used.");
		System.err.println(
				"  " + emit.unused_non_term + " non terminal" + plural(emit.unused_term) + " declared but not used.");
