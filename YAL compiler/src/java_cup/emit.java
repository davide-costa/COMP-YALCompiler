 ...
 * code to get the next token. start_production - the start production for the
...
public class emit
{
	...
	public static void symbols(PrintStream out, boolean emit_non_terms)
	{
		...
		/* class header */
		out.println("/** JavaCup generated class containing symbol constants. */");
		...
		for (Enumeration e = terminal.all(); e.hasMoreElements();)
		{
			...
			out.println("  static final int " + term.name() + " = " + term.index() + ";");
		}
	 ...
	 *            the start production of the grammar.
	 */
	protected static void emit_action_code(PrintStream out, production start_prod) throws internal_error
	{
		production prod;
		...
		/* class header */
		out.println();
		out.println("/** JavaCup generated class to encapsulate user supplied action code.*/");
		...
		for (Enumeration p = production.all(); p.hasMoreElements();)
		{
			...
			if (debug_grammar != null)
				out.println("             " + debug_grammar + "(\"" + prod.to_simple_string() + "\");");

			/* create the result symbol */
			out.println("              " + pre("result") + " = new " + prod.lhs().the_symbol().stack_type() + "(/*"
	 ...
	 */
	protected static void do_action_table(PrintStream out, parse_action_table act_tab, boolean compact_reduces)
	 ...
	 */
	protected static void do_reduce_table(PrintStream out, parse_reduce_table red_tab)
	{
		lalr_state goto_st;
		parse_action act;
	 ...
	 */
	public static void parser(PrintStream out, parse_action_table action_table, parse_reduce_table reduce_table,
			int start_st, production start_prod, boolean compact_reduces) throws internal_error
	{
		long start_time = System.currentTimeMillis();
