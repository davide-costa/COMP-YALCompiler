 ...
 * next token. Once end of input is reached the EOF token is returned on every
 * subsequent call.
 * <p>
 ...
 * next_token() to get each token) this class provides simple error and warning
...
public class lexer
{
	...
	/**
	 * Table of keywords. Keywords are initially treated as identifiers. Just before
	 * they are returned we look them up in this table to see if they match one of
	 * the keywords. The string of the name is the key here, which indexes Integer
	...
	/**
	 * Table of single character symbols. For ease of implementation, we store all
	 * unambiguous single character tokens in this table of Integer objects keyed by
	 * Integer objects with the numerical value of the appropriate char (currently
	 * Character objects have a bug which precludes their use in tables).
	 */
	...
	/**
	 * Initialize the scanner. This sets up the keywords and char_symbols tables and
	 * reads the first two characters of lookahead.
	 */
	...
	/**
	 * Advance the scanner one character in the input stream. This moves next_char2
	 * to next_char and then reads a new next_char2.
	 */
	...
	/**
	 * Emit an error message. The message will be marked with both the current line
	 * number and the position in the line. Error messages are printed on standard
	 * error (System.err).
	 * 
	 * @param message
	...
	/**
	 * Emit a warning message. The message will be marked with both the current line
	 * number and the position in the line. Messages are printed on standard error
	 * (System.err).
	 * 
	 * @param message
	 *            the message to print.
	...
	/**
	 * Handle swallowing up a comment. Both old style C and new style C++ comments
	 * are handled.
	 */
	 ...
	 * ":}" inside a code string). The routine returns an str_token object suitable
	...
	/**
	 * Process an identifier. Identifiers begin with a letter, underscore, or dollar
	 * sign, which is followed by zero or more letters, numbers, underscores or
	 * dollar signs. This routine returns an str_token suitable for return by the
	 * scanner.
	 */
	 ...
	 * consumes sufficient characters to determine the next input token and returns
	 * it. To help with debugging, this routine actually calls real_next_token()
	 * which does the work. If you need to debug the parser, this can be changed to
	 * call debug_next_token() which prints a debugging message before returning the
	 * token.
	 */
	 ...
	 * routine, prints a message on System.out indicating what the token is, then
	 * returns it.
	 */
