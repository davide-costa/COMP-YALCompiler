...
public class production
{
			...
			throws internal_error
	{
		...
		action_str = translate_labels(rhs_parts, rhs_l, action_str, lhs_sym.stack_type());
	 ...
	 * Symbols appearing in the RHS correspond to objects found on the parse stack
	 * at runtime. The code to access them, becomes code to access an object at the
	 * appropriate offset from the top of the stack, and then cast that to the
	 * proper type.
	 *
	 * @param id_str
	 *            the name of the id to be translated.
	 * @param act_pos
	 *            the original position of the action it appears in.
	 * @param label_map
	 *            a hash table mapping labels to positions in the RHS.
	 * @param type_map
	 *            a hash table mapping labels to declared symbol types.
	 */
	protected String label_translate(String id_str, /* the id string we are (possibly) translating */
			...
			Hashtable label_types)/* map from labels to stack types */
	{
		...
		if (label_pos.intValue() > act_pos)
		{
			...
			System.err.println("*** Label \"" + id_str + "\" appears in action before it appears in production");
			lexer.error_count++;
		...
		/*
		 * calculate the stack offset as the difference in position from label to action
		 * minus one
		 */
		offset = (act_pos - label_pos.intValue()) - 1;

		/* translation is properly cast element at that offset from TOS */
		return "(/*" + id_str + "*/(" + label_type + ")" + emit.pre("stack") + ".elementAt(" + emit.pre("top") + "-"
				+ offset + "))";
	 ...
	 * @param act_pos
	 *            the position that the action originally held in the production.
	 * @param label_map
	 *            a hash table mapping labels to positions in the RHS.
	 * @param type_map
	 *            a hash table mapping labels to declared symbol types.
	 */
	protected String action_translate(String act_string, /* the action string */
			...
			Hashtable label_types) /* map from labels to symbol stack types */
	{
		...
		if (act_string == null || act_string.length() == 0)
			return act_string;
	 ...
	 */
	protected String translate_labels(production_part rhs[], int rhs_len, String final_action, String lhs_type)
	{
		...
		for (pos = 0; pos < rhs_len; pos++)
		{
			if (rhs[pos].is_action())
			{
				...
				act_part.set_code_string(action_translate(act_part.code_string(), pos, label_map, label_types));
			}
		}
	...
	protected int merge_adjacent_actions(production_part rhs_parts[], int len)
	{
		...
		for (from_loc = 0; from_loc < len; from_loc++)
		{
			...
			if (to_loc != from_loc)
			{
				...
				{
					/* merge */
					rhs_parts[to_loc] = new action_part(((action_part) rhs_parts[to_loc]).code_string()
	...
	protected void remove_embedded_actions() throws internal_error
	{
		...
		for (int act_loc = 0; act_loc < rhs_length(); act_loc++)
			if (rhs(act_loc).is_action())
			{
				...
				new_prod = new action_production(this, new_nt, null, 0, ((action_part) rhs(act_loc)).code_string());
