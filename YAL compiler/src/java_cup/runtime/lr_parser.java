 ...
 * and the current lookahead token as indexes) to determine whether to shift or
 ...
 * lookahead token as indexes) to determine a new state to go to. The parser
 ...
 * <dd>This determines how many tokens past the point of an error must be parsed
 * without error in order to consider a recovery to be valid. This defaults to
 * 3. Values less than 2 are not recommended.
 * <dt>void report_error(String message, Object info)
 * <dd>This method is called to report an error. The default implementation
 * simply prints a message to System.err and ignores its second parameter. This
...
public abstract class lr_parser
{
	...
	/**
	 * The default number of tokens after an error we much match to consider it
	 * recovered from.
	 */
	...
	/**
	 * The number of tokens after an error we much match to consider it recovered
	 * from.
	 */
	 ...
	 * the scanner. This is called before the parser requests the first token. Here
	...
	/**
	 * Get the next token from the input (supplied by generated subclass). Once end
	 * of file has been reached, all subsequent calls to scan should return an EOF
	 * token (which is symbol number 0). This method is supplied by the generator
	 * using using the code declared in the "scan with" clause.
	 */
	 ...
	 *            the current lookahead token.
	 */
	public void unrecovered_syntax_error(token cur_token) throws java.lang.Exception
	{
		report_fatal_error("Couldn't repair and continue parse", null);
	}
	 ...
	 *
	 * @param shift_tkn
	 *            the token being shifted onto the stack.
	 */
	 ...
	 * "lookahead" or "parse ahead" tokens are read into a buffer. The size of this
	 * buffer is determined by error_sync_size() and determines how many tokens
	 * beyond the error must be matched to consider the recovery a success. Next, we
	 * begin to discard tokens in attempt to get past the point of error to a point
	 * where we can continue parsing. After each token, we attempt to "parse ahead"
	 * though the buffered lookahead tokens. The "parse ahead" process simulates
	 * that actual parse, but does not modify the real parser's configuration, nor
	 * execute any actions. If we can parse all the stored tokens without error,
	...
	/**
	 * Reset the parse ahead input to one token past where we started error recovery
	 * (this consumes one new token from the real input).
	 */
	...
	protected boolean try_parse_ahead(boolean debug) throws java.lang.Exception
	{
		...
		for (;;)
		{
			...
			if (act > 0)
			{
				...
				if (debug)
					debug_message("# Parse-ahead shifts token #" + cur_err_token().sym + " into state #" + (act - 1));
